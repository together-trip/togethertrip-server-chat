...
이 리포는 TogetherTrip의 Spring Boot WebSocket 채팅 서버다. 에이전트와 Codex는 이 파일을 우선 읽고, `docs/agents/repo-context.md`, `docs/agents/quality-gates.md`, `.codex/setup.md`를 함께 따른다.

## 작업 원칙

- 문서와 산출물은 한국어로 작성한다. 명령어, 코드 식별자, 파일 경로, 라이브러리명은 원문을 유지한다.
- 구현 전에는 채팅방 접근 권한, WebSocket 세션 인증, 메시지 저장/조회 정책을 확인한다.
- 채팅 bounded context를 독립적으로 유지하고, 외부 시스템 연동은 서비스 계층에서 명시적으로 분리한다.
- 여행별 채팅방 접근 제어와 rate limit은 보안 검토 대상으로 본다.
